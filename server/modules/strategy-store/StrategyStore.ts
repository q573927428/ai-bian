...
export class StrategyStore {
  ...
  async rollbackStrategy(
    ...
    try {
      ...
      // 保存回滚后的策略
      await this.saveStrategyWithoutTradeRecords(rolledBackStrategy)
  ...
  async addTradeRecord(strategyId: StrategyId, record: Omit<import('../../../types/strategy').TradeRecord, 'id' | 'strategyId' | 'strategyVersion'>): Promise<string | null> {
    logger.warn('StrategyStore', 'addTradeRecord 已弃用，使用全局交易历史文件')
    return null
  }

  /**
   * 更新交易记录（已弃用，使用全局交易历史文件）
   */
  async updateTradeRecord(strategyId: StrategyId, recordId: string, updates: Partial<import('../../../types/strategy').TradeRecord>): Promise<boolean> {
    logger.warn('StrategyStore', 'updateTradeRecord 已弃用，使用全局交易历史文件')
    return false
  }

  /**
   * 保存策略时不包含 tradeRecords
   */
  private async saveStrategyWithoutTradeRecords(strategy: Strategy): Promise<void> {
    // 创建不包含 tradeRecords 的副本
    const { tradeRecords, ...strategyWithoutRecords } = strategy as any
    const filePath = getStrategyFilePath(strategy.id)
    await writeFile(filePath, JSON.stringify(strategyWithoutRecords, null, 2), 'utf-8')
  }
  ...
  async getTradeRecords(strategyId: StrategyId, limit?: number, symbol?: string): Promise<import('../../../types/strategy').TradeRecord[]> {
    try {
      ...
      let records = globalRecords.sort((a, b) => 
