...
export class MultiStrategyAIAnalyzer {
  ...
  private buildFullPrompt(
    ...
    return `
      ...
      预计K线结束成交量：${(volume / candleProgress).toFixed(2)}
      ...
      本次分析使用指标：${enabledIndicatorsList.join(', ')}
      ` : ''}
      ...
      ## 五、执行规则
      1. 完全依据用户策略规则决策
      2. 若用户策略未明确方向，使用基础EMA排列判断（短周期EMA>长周期EMA=LONG，反之SHORT）
      3. 不要怕给方向！最终决策是否交易的是置信度，而不是方向本身
      4. 即使只有微弱倾向，也要给出 LONG/SHORT，用置信度来表示把握大小
      5. 只有用户策略里面明确要求给IDLE时，才给IDLE，其他时候必须给出LONG/SHORT
      6. reasoning必须使用中文！
  ...
  private async buildSystemPrompt(): Promise<string> {
    return `
      ...
      4. 即使只有微弱倾向，也要给出 LONG/SHORT，用置信度来表示把握大小
      5. 只有用户策略里面明确要求给IDLE时，才给IDLE，其他时候必须给出LONG/SHORT
      6. 永远输出干净、标准、可解析的JSON，reasoning必须使用中文
      
      --------------------------
      【输出铁律】
      
      - 只返回JSON，无任何多余内容
      - 严格包含：direction / confidence / reasoning
      - 必须基于数据，且包含具体数值
      - 客观、机械、稳定、一致
    `;
  }
